...
USER root
RUN mkdir /tmp/renamed
RUN chmod -R 777 /tmp/renamed
...
##Need to build a new image, lo tienes abajo :
    ...
    # Start Docker deamon
    # docker build -t iam-sftp-adapter:iam_4.7-zip-rec .
    # Tag it and push to quay
    # docker tag iam-sftp-adapter:iam_4.7-zip-rec quay.io/msentissi/iam-sftp-adapter:iam_4.7-zip-rec
    # docker login registry.redhat.io 
    # docker push quay.io/msentissi/iam-sftp-adapter:iam_4.7-zip-rec
    # OR tag it and push to dockerhub
    #   docker push msentissi/iam-sftp-adapter:iam_4.7-zip-rec

    # docker run --rm -ti iam-sftp-adapter:iam_4.7-zip-rec
    # docker run --env JAVA_OPTS="-Xms256m -Xmx5024m" --network host --rm -ti iam-sftp-adapter:iam_4.7-zip-rec
    # docker run --rm -ti --network="host" -e sFTP_HOST="130.24.80.145" -e sFTP_PWD="123456" iam-sftp-adapter:iam_4.7-zip-rec
